...
def ListaDivisibles(numero, tope):
    ...
    lista = []
    if(numero < tope):
        for i in range(1,int(tope/numero)+1):
    ...
    if type(lista) == list:
        for i in range(len(lista)):
            if(type(lista[i]) == list):
                listaAux = ListaDeListas(lista[i])
                for j in range(len(listaAux)):
                    nuevaLista.append(listaAux[j])
            else:
...
def ListaRepetidos(lista):
    ...
    if(type(lista) == list):
        ...
        while(i<len(lista)):
            repetidos = 1
            j = i+1
            while(j<len(lista) and j>i):
                if(lista[i] == lista[j]):
                    repetidos += 1
                    lista.pop(j)
                j += 1
            listaRepetidos.append((lista[i],repetidos))
            i += 1
...
def ClaseVehiculo(tipo, color):
    ...
    class Vehiculo:
        ...
        def Acelerar(self, aumentaVelocidad):
            self.velocidad += aumentaVelocidad
            if self.velocidad < 0:
                self.velocidad = 0
    ...
    vehiculo = Vehiculo(tipo, color, 0.0)
    return vehiculo

def OrdenarDiccionario(diccionario_par, clave, ascendente=True):
    '''
    ...
    if(type(diccionario_par) == dict):
        ...
        if(existeClave):
            if(ascendente == True):
                for i in range(len(diccionario_par[clave])-1):
                    minimo = i
                    for j in range(i+1,len(diccionario_par[clave])):
                        if diccionario_par[clave][j] < diccionario_par[clave][minimo]:
                            minimo = j
                    for j in range(len(claves)):
                        aux = diccionario_par[claves[j]][i]
                        diccionario_par[claves[j]][i] = diccionario_par[claves[j]][minimo]
                        diccionario_par[claves[j]][minimo] = aux
            else:
                for i in range(len(diccionario_par[clave])-1):
                    maximo = i
                    for j in range(i+1,len(diccionario_par[clave])):
                        if diccionario_par[clave][j] > diccionario_par[clave][maximo]:
                            maximo = j
                    for j in range(len(claves)):
                        aux = diccionario_par[claves[j]][i]
                        diccionario_par[claves[j]][i] = diccionario_par[claves[j]][maximo]
                        diccionario_par[claves[j]][maximo] = aux
